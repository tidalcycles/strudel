...
You can also change the speed of a pattern to match a given number of steps per cycle, with the `mesh` function:

<MiniRepl client:idle tune={`stepcat(sound("bd hh mt lt").expand(2), sound("cp cp")).mesh(8)`} />

<MiniRepl client:idle tune={`stepcat(sound("bd hh mt lt").expand(4), sound("cp cp")).mesh(8)`} />
...
This results in a pretty dense pattern, because the different expanded versions are squashed (stepwise) into a single cycle. `mesh` is again handy here for slowing down the pattern to a particular number of steps per cycle:

<MiniRepl client:idle tune={`sound("bd hh mt lt").expand("3 2 1 1 2 3").mesh(8)`} />

## Stepwise functions

### mesh

<JsDoc client:idle name="mesh" h={0} />
