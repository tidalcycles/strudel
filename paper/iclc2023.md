...
Strudel is split into multiple packages, allowing granular reuse in other
...
```tidal
iter 4 $ every 3 (||+ "10 20") (n "0 1 3") # s "triangle" # crush 4
```

```js
iter(4, every(3, add("10 20"), n("60 61 63").s("triangle").crush(4)))

// simplified:
n("60 61 63").every(3,add("10 20")).iter(4).s("sawtooth").crush(4)
