...
2. While the REPL is running, the `Scheduler` queries the active `Pattern` by a regular interval, generating `Events` (also called `Haps` in Strudel) for the next slice of time.
